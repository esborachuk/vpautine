 ...
 * @version 		$Id: like.class.php 5300 2013-02-01 08:54:25Z Miguel_Espinoza $
 */
class Like_Service_Like extends Phpfox_Service 
{
	...
	public function getActionsFor($sItemTypeId, $iItemId)
	{
		...
		$aActions = $this->database()->select('a.*, u.user_name, u.full_name, f.friend_id as is_friend, l.type_id as like_type_id, l.time_stamp as like_time_stamp')
            ...
            ->group('a.action_id')
			->where('a.item_type_id = "'. $oParse->clean($sItemTypeId) .'" AND a.item_id = ' . (int)$iItemId)
			->execute('getSlaveRows');
		...
		foreach ($aActions as $aAction)
		{
			...
			if ($aAction['is_friend'] > 0)
			{
				if (Phpfox::getParam('feed.total_likes_to_display') > $aOut[$aAction['action_type_id']]['votes_from_friends'])
				{
					// find the user id				
