					...
					<div class="drop_data_action">
						...
						<div class="js_drop_data_button" id="drop_down_{$aFriend.request_id}">
							<ul class="table_clear_button">
								<li>
<!--                                    <span class="title-button" style="opacity: 0;">{phrase var='friend.confirm'}</span>-->
                                    <input type="button" name="" value="+" class="button" onclick="$(this).parents('.drop_data_action').find('.js_drop_data_add').show(); {if $aFriend.relation_data_id > 0} $.ajaxCall('custom.processRelationship', 'relation_data_id={$aFriend.relation_data_id}&amp;type=accept&amp;request_id={$aFriend.request_id}'); {else} $.ajaxCall('friend.processRequest', 'type=yes&amp;user_id={$aFriend.user_id}&amp;request_id={$aFriend.request_id}&amp;inline=true'); {/if}" />
                                </li>
								<li>
<!--                                    <span class="title-button" style="opacity: 0;">{phrase var='friend.deny'}</span>-->
                                    <input type="button" name="" value="-" class="button button_off" onclick="$(this).parents('.drop_data_action').find('.js_drop_data_add').show(); {if $aFriend.relation_data_id > 0} $.ajaxCall('custom.processRelationship', 'relation_data_id={$aFriend.relation_data_id}&amp;type=deny&amp;request_id={$aFriend.request_id}'); {else} $.ajaxCall('friend.processRequest', 'type=no&amp;user_id={$aFriend.user_id}&amp;request_id={$aFriend.request_id}&amp;inline=true'); {/if}" />
                                </li>
							</ul>
							<div class="clear"></div>
						</div>
