...
<div id="js_notification_holder">
	<ul class="notification_holder">
		...
		{foreach from=$aSubNotifications item=aNotification}
            ...
            <li id="js_notification_{$aNotification.notification_id}" class="{if !$aNotification.is_seen} is_new{/if} not-information">
                <table class="notification-messages">
                    <tr>
                        <td class='not-user-image'>
                            {if !isset($aNotification.no_profile_image)}
                                {img user=$aNotification max_width='50' max_height='50' suffix='_50_square' no_link=false}
                            {/if}
                        </td>
                        <td class='not-message'>
                            <a href="{$aNotification.link}" class="main_link{if !$aNotification.is_seen} is_new{/if}">
                                {$aNotification.message}
                            </a>
                        </td>
                        <td class='not-posted-time'>
                            <span class="extra_info">{$aNotification.time_stamp|convert_time}</span>
                        </td>
                        <td class='not-del-button'>
                            <span class="notification_delete">
                                <a href="#" class="js_hover_title" onclick="$.ajaxCall('notification.delete', 'id={$aNotification.notification_id}'); return false;">
                                    ...
                                    <span class="js_hover_info">{phrase var='notification.delete_this_notification'}</span>
                                </a>
                            </span>
                        </td>
                    </tr>
	...
	<ul class="table_clear_button" id="js_notification_list_delete">
		<li>
            <input type="button" value="{phrase var='notification.delete_all_notifications'}" class="button"
                   onclick="$Core.processForm('#js_notification_list_delete'); $(this).ajaxCall('notification.removeAll'); return false;" />
        </li>
