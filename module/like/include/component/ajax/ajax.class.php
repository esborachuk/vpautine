 ...
 * @author  		Raymond_Benc
 * @package 		Phpfox_Ajax
 * @version 		$Id: ajax.class.php 5301 2013-02-01 09:14:19Z Miguel_Espinoza $
 */
class Like_Component_Ajax_Ajax extends Phpfox_Ajax
{
	...
	public function doAction()
	{
		...
		$aLikes['feed_like_phrase'] = Phpfox::getService('feed')->getPhraseForLikes($aT);
		...
		{			
			...
			$this->template()->assign(array(
				'aFeed' => array(
					...
					'feed_like_phrase' => $aLikes['feed_like_phrase']
					)
				)			
			);
	    ...
	    if (Phpfox::getService('like.process')->removeAction($this->get('action_type_id'), $this->get('item_type_id'), $this->get('item_id'), $this->get('module_name') ))
	    {
            ...
            $this->template()->assign(array(
			    'aFeed' => array(
				    ...
				    'likes' => $aLikes,
				    'feed_id' => $this->get('item_id'),
				    'feed_like_phrase' => $aT['feed_like_phrase']
				    )
			    )			
		    );
