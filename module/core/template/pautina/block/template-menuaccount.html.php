...
?>
    ...
    {foreach from=$aRightMenus key=iKey item=aMenu}
        <li><a {if Phpfox::isUser() && $aMenu.url == 'user.setting'}class="menu_settings" style="background-image='{$aGlobalUser}'"{/if} href="{url link=$aMenu.url}"{if isset($aMenu.children) && count($aMenu.children) && is_array($aMenu.children)} class="has_drop_down no_ajax_link"{/if}>{phrase var=$aMenu.module'.'$aMenu.var_name}{if isset($aMenu.suffix)}{$aMenu.suffix}{/if} </a>
